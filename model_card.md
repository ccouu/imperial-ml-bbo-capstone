...
*   **Metric Misalignment in Low Dimensions:** While tree-based regressors (XGBoost/Decision Trees) achieved excellent cross-validation MSE (`< 0.3`) in 2D functions, they underperformed compared to peers using standard Gaussian Processes. This revealed a critical limitation: low MSE does not guarantee optimization success. Trees create flat, step-wise predictive surfaces that destroy the smooth gradients required by acquisition functions, causing the search to stall on artificial plateaus rather than converging smoothly to the peak.
## Trade-offs
